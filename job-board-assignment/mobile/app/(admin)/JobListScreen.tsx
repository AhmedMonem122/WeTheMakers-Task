...
type Job = { id: string; title: string; location: string; salary?: number; description?: string }

export default function AdminJobListScreen() {
  ...
  if (error) return (
    <View style={styles.center}><Text>Error loading jobs</Text></View>
  )

  return (
    <View style={styles.container}>
      <Text style={styles.title}>Admin Jobs</Text>
      <Button title="Create Job" onPress={() => navigation.navigate('AdminJobForm' as any)} />
      <FlatList
        data={data as Job[]}
        ...
        renderItem={({ item }: { item: Job }) => (
          <View style={styles.card}>
            ...
            <Text style={styles.meta}>{item.location}{item.salary ? ` • ${item.salary}` : ''}</Text>
            <Text style={styles.desc} numberOfLines={2}>{item.description}</Text>
            <View style={styles.actions}>
              <TouchableOpacity onPress={() => navigation.navigate('AdminJobForm' as any, { id: item.id })}>
